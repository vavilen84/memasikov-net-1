...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "memasikov.net" do |company|
	company.vm.box = "memasikov.net"
	# Configure some Virtual Box params
	company.vm.provider :virtualbox do |company|
		company.customize ["modifyvm", :id, "--name", "memasikov.net"]
	...
	company.vm.box_url = "https://cloud-images.ubuntu.com/vagrant/trusty/current/trusty-server-cloudimg-amd64-vagrant-disk1.box"
        ...
        if host_os =~ /linux/ or host_os =~ /darwin/
			company.vm.synced_folder "/var/www/memasikov.net", "/var/www/memasikov.net", type: "nfs", version: 3, nfs_udp: 1, mount_options: ["rw", "tcp", "nolock", "noacl", "sync"]
        else
			company.vm.synced_folder "/var/www/memasikov.net", "/var/www/memasikov.net", owner: "www-data", group: "www-data"
        end
